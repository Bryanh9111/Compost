...
} from "../../../compost-core/src/metacognitive/action-timeline";
...
export function registerDid(program: Command): void {
  program
    .command("did")
    .description("Summarize action_log activity for a date or time window")
    .argument(
      "<when>",
    ...
    .action((when, opts) => {
      const db = openDb();
      try {
        const report = summarizeActionTimeline(db, when, {
        ...
        process.stdout.write(formatActionTimeline(report));
      } finally {
        db.close();
      }
    });
