...
} from "../../../compost-core/src/metacognitive/action-patterns";
...
export function registerPatterns(program: Command): void {
  program
    .command("patterns")
    .description("Detect read-only deterministic patterns over action_log")
    .argument(
      "<when>",
    ...
    .action((when, opts) => {
      const db = openDb();
      try {
        const report = detectActionPatterns(db, when, {
          project: opts.project,
          sourceSystem: opts.source,
          limit: opts.limit,
          patternLimit: opts.patternLimit,
          now: opts.now,
        });
        ...
        process.stdout.write(formatActionPatterns(report));
      } finally {
        db.close();
      }
    });
