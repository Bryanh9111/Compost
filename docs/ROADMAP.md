...
- Phase 2-3 (1-3 months): action_log schema (0021 landed) + capture expansion (zsh / git / Obsidian) + `compost cover <topic>` coverage audit + `compost route <question>` artifact routing + `compost did <date>` action timeline aggregation. Richer ledger for (a) to draw from.
