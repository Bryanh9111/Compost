...
Implementation baseline after this turn: `fda433d` lands the `action_log` schema foundation; `c00db8d` restores root typecheck and the full test suite while preserving the v4 freeze defaults; `d406830` adds the action processor that lifts drained observations into `action_log` for the metacognitive timeline. The D2-3 zsh capture slice adds `compost capture zsh`, a local `preexec_functions`/`precmd_functions` hook, PII scrubbing before outbox write, and zsh normalization in `action_log`.
