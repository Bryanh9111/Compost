...
```
          ...
          +-------+-------+-------------------+-------------+
          |       |          |                 |
      local file  web URL  zsh/git/Obsidian   Claude Code hook
          |       |         capture           |
          v       v             v             v
    +--------------------------------------+
...
packages/
  ...
  compost-cli/             # CLI: add, query, ask, capture, cover, route, did, reconcile, doctor
...
- **763 tests**, 0 failures
- **22 SQL migrations** (observations, chunks, facts, wiki, outbox, fact_links, user-model schema in 0015, open_problems in 0016, crawl_queue in 0017, reasoning scheduler state in 0020, action_log in 0021, Engram context backfill in 0022)
